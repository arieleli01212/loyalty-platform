...
import { scan, clearTokens, ScanAction, ScanResponse, ApiError, NetworkError } from '../api/client'
...
type ScanState =
  | { kind: 'scanning' }
  | { kind: 'loading'; barcodeToken: string }
  | { kind: 'success'; result: ScanResponse }
  | { kind: 'error'; message: string }

function errorMessage(err: unknown): string {
  if (err instanceof ApiError) {
    if (err.status === 429) return 'Too soon — wait before stamping again'
    if (err.status === 400) return 'No reward available to redeem'
    if (err.status === 404) return 'Card not recognized'
    if (err.status === 403) return 'Card belongs to another business'
...
export default function ScannerPage({ onLogout }: Props) {
  ...
  const handleScan = useCallback(
    async (barcodeToken: string) => {
      // Ignore scans while loading or already showing result
      setState((prev) => {
        if (prev.kind !== 'scanning') return prev
        return { kind: 'loading', barcodeToken }
      })
      ...
      try {
        ...
        // If 401 after refresh also failed, kick to login
    ...
    <div className="flex min-h-screen flex-col bg-slate-900">
      ...
      <header className="flex items-center justify-between px-4 py-3 bg-slate-800 border-b border-slate-700">
        ...
        <button
          ...
          className="rounded-lg border border-slate-600 px-4 py-2 text-sm font-medium text-slate-300 hover:bg-slate-700 active:bg-slate-600 transition"
        >
          Logout
        </button>
      </header>
...
function SuccessCard({
  ...
  return (
    <div className="mx-4 mt-3 rounded-2xl border border-emerald-700 bg-emerald-900/40 px-6 py-6">
      ...
      <button
        onClick={onNext}
        className="w-full rounded-xl bg-emerald-600 py-4 text-lg font-semibold text-white hover:bg-emerald-500 active:bg-emerald-700 transition"
      >
        Scan next
      </button>
    </div>
...
function ErrorCard({ message, onNext }: { message: string; onNext: () => void }) {
  return (
    <div className="mx-4 mt-3 rounded-2xl border border-red-700 bg-red-900/40 px-6 py-6">
      ...
      <button
        onClick={onNext}
        className="w-full rounded-xl bg-slate-700 py-4 text-lg font-semibold text-white hover:bg-slate-600 active:bg-slate-800 transition"
      >
        Try again
      </button>
    </div>
